<div class="modal fade" id="social-media-edit-modal">
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5 fw-bold">{{ __('Edit Social Media') }}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="personal-info">
                    <form action="" method="post"
                        enctype="multipart/form-data" class="add-brand-form pt-0 ajaxform_instant_reload mediaUpdateForm">
                        @csrf
                        @method('put')
                        <div class="row">
                            <div class="mt-3 col-lg-12">
                                <label class="custom-top-label">{{ __('Title') }}</label>
                                <input type="text" name="title" id="media_title" required placeholder="{{ __('Enter media title') }}" class="form-control" />
                            </div>
                            <div class="mt-3 col-lg-12">
                                <label class="custom-top-label">{{ __('Url') }}</label>
                                <input type="text" name="url" id="media_url" required placeholder="{{ __('Enter media url') }}" class="form-control" />
                            </div>
